...
create index IX_vtables_module_name on vtables (module_name))");
	...
	db->exec(R"(create table cotype_function (
clsid blob not null, 
iid blob not null,
ordinal integer not null,
name text not null,
...
std::unique_ptr<SQLite::Database> cometa::open_db(const fs::path& path, IDebugControl4* dbgcontrol) {
	...
	log.log_info(std::format(L"Open metadata database from '{0}'", path.c_str()));
	return db;
}


cometa::cometa(IDebugControl4* dbgcontrol, bool is_wow64, const fs::path& db_path, bool create_new) :
	_logger{ dbgcontrol }, _dbgcontrol(dbgcontrol), _is_wow64{ is_wow64 },
	...
	{
		fill_synthetic_symbols();
	}
}

void cometa::fill_synthetic_symbols() {
	int cntsymbol = 0;
	for (auto& [module_name, clsid, iid, vtbl] : find_vtables_all()) {
		auto iid_name{ resolve_type_name(iid) };

		for (auto& [func_name, iidfunc, vtblfunc, address] : find_function_by_clsid(clsid))
		{
			if (vtblfunc == vtbl && iidfunc == iid)
			{
				cntsymbol++;
				NewSyntheticSymbolInModule(module_name, address, sizeof(ULONG_PTR), func_name);
			}
		}

	}
	_logger.log_info(std::format(L"fetched synthetic symbol count:{0}",  cntsymbol));
}
...
void cometa::fill_known_iids() {
	...
	insert_cotype_method(comethod{ __uuidof(IUnknown), L"QueryInterface", 0, CC_STDCALL, std::nullopt, L"HRESULT" ,0 });
	...
	insert_cotype_method(comethod{ __uuidof(IDispatch), L"GetTypeInfoCount", 0, CC_STDCALL, std::nullopt, L"HRESULT" ,0 });
	...
	insert_cotype_method(comethod{ __uuidof(IDispatch), L"GetTypeInfo", 1, CC_STDCALL, std::nullopt, L"HRESULT" ,0 });
	...
	insert_cotype_method(comethod{ __uuidof(IDispatch), L"GetIDsOfNames", 2, CC_STDCALL, std::nullopt, L"HRESULT" ,0 });
	...
	insert_cotype_method(comethod{ __uuidof(IDispatch), L"Invoke", 3, CC_STDCALL, std::nullopt, L"HRESULT" ,0 });
	...
	_known_iids.insert( __uuidof(IUnknown) );
	_known_iids.insert( __uuidof(IDispatch) );
}
...
void cometa::insert_cotype_method(const comethod& method) {
	...
	SQLite::Statement stmt{ *_db, R"(insert or replace into cotype_methods (iid, ordinal, name, dispid, callconv, return_type,oVft)
    values (:iid, :ordinal, :name, :dispid, :callconv, :return_type, :oVft))" };
	...
	stmt.bind(":oVft", static_cast<int>(method.oVft));
	stmt.exec();
}

void cometa::insert_cotype_function(const cofunction& func) {
	if (_known_iids.contains(func.iid)) {
		return;
	}

	assert(_db);

	auto name_u8{ to_utf8(func.name) };
	auto return_type_u8{ to_utf8(func.return_type) };

	SQLite::Statement stmt{ *_db, R"(insert or replace into cotype_function (clsid,iid, ordinal, name, dispid, callconv, return_type,oVft,address,vtable)
    values (:clsid,:iid, :ordinal, :name, :dispid, :callconv, :return_type, :oVft,:address,:vtable))" };
	stmt.bindNoCopy(":clsid", &func.clsid, sizeof(GUID));
	stmt.bindNoCopy(":iid", &func.iid, sizeof(GUID));
	stmt.bind(":ordinal", func.ordinal);
	stmt.bindNoCopy(":name", name_u8);
	if (func.dispid) {
		stmt.bind(":dispid", static_cast<int>(*func.dispid));
	}
	else {
		stmt.bind(":dispid");
	}

	stmt.bind(":callconv", static_cast<int>(func.callconv));
	stmt.bindNoCopy(":return_type", return_type_u8);
	stmt.bind(":oVft", static_cast<int>(func.oVft));
	stmt.bind(":address", static_cast<int>(func.address));
	stmt.bind(":vtable", static_cast<int>(func.vtable));
	stmt.exec();
}
...
BOOLEAN cometa::NewSyntheticSymbol(ULONG64 offset, ULONG64 sym_size, const std::wstring& sym_name)
{

	wil::com_ptr_t<IDebugSymbols3> g_ExtSymbols;
	RETURN_IF_FAILED(_dbgcontrol->QueryInterface(__uuidof(IDebugSymbols3), g_ExtSymbols.put_void()));


	std::map<ULONG64, synthetic_symbol>::iterator it = g_synthetic_symbols_list.find(offset);
	if (it != g_synthetic_symbols_list.end()) {
		g_ExtSymbols->RemoveSyntheticSymbol(&it->second.id);
	}


	DEBUG_MODULE_AND_ID id = { 0 };
	if (SUCCEEDED(g_ExtSymbols->AddSyntheticSymbolWide(offset, (ULONG)sym_size, sym_name.c_str(), DEBUG_ADDSYNTHSYM_DEFAULT, &id))) {
...
BOOLEAN cometa::NewSyntheticSymbolInModule(const std::wstring& mod_name, ULONG64 offset, ULONG64 sym_size, const std::wstring& sym_name)
{
	wil::com_ptr_t<IDebugSymbols3> g_ExtSymbols;
	RETURN_IF_FAILED(_dbgcontrol->QueryInterface(__uuidof(IDebugSymbols3), g_ExtSymbols.put_void()));
	...
	ULONG mod_idx = 0;
	ULONG64 mod_base = 0;
	if (SUCCEEDED(g_ExtSymbols->GetModuleByModuleName2Wide(mod_name.c_str(), 0, DEBUG_GETMOD_NO_UNLOADED_MODULES, &mod_idx, &mod_base)))
	{
		offset += mod_base;
		return  NewSyntheticSymbol(offset, sym_size, sym_name);
	}
...
std::wstring cometa::resolve_module_path_by_name(std::wstring nameorpath) {
	...
	wil::com_ptr_t<IDebugSymbols3> g_ExtSymbols;
    _dbgcontrol->QueryInterface(__uuidof(IDebugSymbols3), g_ExtSymbols.put_void());

	ULONG mod_idx = 0;
	ULONG64 mod_base = 0;
	if (SUCCEEDED(g_ExtSymbols->GetModuleByModuleName2Wide(nameorpath.c_str(), 0, DEBUG_GETMOD_NO_UNLOADED_MODULES, &mod_idx, &mod_base)))
	{
		ULONG NameSize = 0;

	
		if (SUCCEEDED(g_ExtSymbols->GetModuleNameStringWide(DEBUG_MODNAME_IMAGE, mod_idx, mod_base, nullptr, 0, &NameSize))) {
			
			auto names{ std::make_unique<wchar_t[]>(NameSize+1) };
			if (SUCCEEDED(g_ExtSymbols->GetModuleNameStringWide(DEBUG_MODNAME_IMAGE, mod_idx, mod_base, names.get(), NameSize + 1, &NameSize))) {
				std::wstring ret = names.get();
				return ret;
			}
		}
	}
...
HRESULT cometa::index_tlb(std::wstring_view tlb_path) {
	...
	assert(_db);
	std::wstring tlb_path_raw = resolve_module_path_by_name(tlb_path.data());
	wil::com_ptr_t<ITypeLib> typelib{};
	RETURN_IF_FAILED(::LoadTypeLibEx(tlb_path_raw.c_str(), REGKIND_NONE, typelib.put()));
	ULONG64 mod = (ULONG64)(LoadLibraryExW(tlb_path_raw.c_str(), NULL, DONT_RESOLVE_DLL_REFERENCES));
	if (!mod)
	{
		return E_NOTIMPL;
	}
	...
	for (UINT type_num = 0; type_num < types_len; type_num++) {
			...
			for (int ordinal = 0; ordinal < typeattr->cFuncs; ) {
				...
				if (auto names_v{ typelib::get_comethod_names(typeinfo.get(), fd.get()) }; std::holds_alternative<HRESULT>(names_v)) {
					...
					if (kind == cotype_kind::DispInterface && result_vt != VT_HRESULT && result_vt != VT_VOID) {
						...
						insert_cotype_method({ typeattr->guid, method_name, ordinal, fd->callconv, dispid, typelib::vt_to_string(VT_HRESULT) ,fd->oVft });
					}
					else {
						insert_cotype_method({ typeattr->guid, method_name, ordinal, fd->callconv, dispid, get_type_name(&fd->elemdescFunc.tdesc) ,fd->oVft });
					}
		...
		case TKIND_COCLASS: {
			...
			for (unsigned short i = 0; i < typeattr->cImplTypes; i++)
			{
				wil::com_ptr_t <ITypeInfo> pIRefTypeInfo{};
				HREFTYPE hRefType;
				//DecodePointer(IMR_RECONVERTSTRING);
				HRESULT hr = typeinfo->GetRefTypeOfImplType(i, &hRefType);
				if (SUCCEEDED(hr))
				{
					hr = typeinfo->GetRefTypeInfo(hRefType, &pIRefTypeInfo);
					if (SUCCEEDED(hr))
					{
						auto get_type_name = [pIRefTypeInfo](const TYPEDESC* tdesc) {
							auto type_name{ typelib::get_type_desc(pIRefTypeInfo.get(), tdesc) };
							return std::holds_alternative<std::wstring>(type_name) ? std::get<std::wstring>(type_name) :
								std::wstring{ typelib::bad_type_name };
						};
						auto funcdesc_deleter = [pIRefTypeInfo](FUNCDESC* fd) { pIRefTypeInfo->ReleaseFuncDesc(fd); };

						auto pRefTypeAttr_res{ typelib::get_typeinfo_attr(pIRefTypeInfo.get()) };
						if (std::holds_alternative<HRESULT>(pRefTypeAttr_res)) {
							continue;
						}

						auto pRefTypeAttr{ std::move(std::get<typelib::typeattr_t>(pRefTypeAttr_res)) };
						if (SUCCEEDED(hr))
						{

							for (int ordinal = 0; ordinal < pRefTypeAttr->cFuncs; ) {
								FUNCDESC* p_fd;
								RETURN_IF_FAILED(pIRefTypeInfo->GetFuncDesc(ordinal, &p_fd));
								typelib::funcdesc_t fd{ p_fd, funcdesc_deleter };

								if (auto names_v{ typelib::get_comethod_names(pIRefTypeInfo.get(), fd.get()) }; std::holds_alternative<HRESULT>(names_v)) {
									ordinal += 1;
									continue;
								}
								else {
									...
									auto kind{ pRefTypeAttr->typekind == TKIND_INTERFACE ? cotype_kind::Interface : cotype_kind::DispInterface };
									std::optional<DISPID> dispid = kind == cotype_kind::DispInterface ? std::make_optional(fd->memid) : std::nullopt;

									assert((SHORT)names.size() == fd->cParams + 1);

									std::vector<std::tuple<std::wstring, CLSID, IID, ULONG64>> _vtables = this->find_vtables_by_clsid_and_iid(typeattr->guid, pRefTypeAttr->guid);
									if (_vtables.size())
									{
										ULONG64 vtbl = std::get<3>(_vtables.at(0));
										ULONG64 pFunction = (ULONG64)(*(ULONG_PTR*)(vtbl + mod + fd->oVft));
										if (pFunction)
										{
											ULONG64 pFunctionAddr = pFunction - mod;
											insert_cotype_function({ typeattr->guid,pRefTypeAttr->guid, method_name, ordinal, fd->callconv, dispid, get_type_name(&fd->elemdescFunc.tdesc) ,fd->oVft,pFunctionAddr,vtbl });
										}
									}
									// TODO: I'm still missing handling of the optional parameters
							...
							add_symbol_by_clsid_and_iid(typeattr->guid, pRefTypeAttr->guid);
						}
					}
...
std::optional<cotype> cometa::resolve_type(const IID& iid) {
	if (_cotype_cache.contains(iid)) {
		return static_cast<std::optional<cotype>>(_cotype_cache.get(iid));
	}
	...
	_cotype_cache.insert(iid,  std::optional<const cotype>(result));
...
std::optional<coclass> cometa::resolve_class(const CLSID& clsid) {
	if (_coclass_cache.contains(clsid)) {
		return  static_cast<std::optional<coclass>>(_coclass_cache.get(clsid));
	}
...
std::vector<std::tuple<std::wstring, IID, ULONG64, ULONG64>> cometa::find_function_by_clsid(const CLSID& clsid) {
	SQLite::Statement query{ *_db, "select clsid,iid, ordinal, name, dispid, callconv, return_type,oVft,address,vtable from cotype_function where clsid = :clsid" };
	query.bindNoCopy(":clsid", &clsid, sizeof(CLSID));
	std::vector<std::tuple<std::wstring, IID, ULONG64, ULONG64>> vfunc{};
	while (query.executeStep()) {
		vfunc.push_back({
			from_utf8(query.getColumn("name").getString()),
			*(reinterpret_cast<const GUID*>(query.getColumn("iid").getBlob())),
			query.getColumn("vtable").getInt64(),
			query.getColumn("address").getInt64()
			});
	}
	return vfunc;
}

std::vector<std::tuple<std::wstring, CLSID, IID, ULONG64>> cometa::find_vtables_by_clsid_and_iid(const CLSID& clsid, const IID& iid) {
	SQLite::Statement query{ *_db, "select module_name,clsid,iid,vtable from vtables where clsid = :clsid and iid = :iid" };
	query.bindNoCopy(":clsid", &clsid, sizeof(CLSID));
	query.bindNoCopy(":iid", &iid, sizeof(IID));
	std::vector<std::tuple<std::wstring, CLSID, IID, ULONG64>> vtables{};
	while (query.executeStep()) {
		vtables.push_back({
			from_utf8(query.getColumn("module_name").getString()),
			*(reinterpret_cast<const GUID*>(query.getColumn("clsid").getBlob())),
...
std::vector<std::tuple<std::wstring, CLSID, IID, ULONG64>> cometa::find_vtables_all() {
	SQLite::Statement query{ *_db, "select module_name,clsid,iid,vtable from vtables" };
	std::vector<std::tuple<std::wstring, CLSID, IID, ULONG64>> vtables{};
	while (query.executeStep()) {
		vtables.push_back({
			from_utf8(query.getColumn("module_name").getString()),
...
void cometa::add_symbol_by_clsid_and_iid(const CLSID& clsid, const IID& iid) {
	SQLite::Statement query{ *_db, "select module_name,clsid,iid,vtable from vtables where clsid = :clsid and iid = :iid" };
	query.bindNoCopy(":clsid", &clsid, sizeof(CLSID));
	query.bindNoCopy(":iid", &iid, sizeof(IID));
	std::vector<std::tuple<std::wstring, CLSID, IID, ULONG64>> vtables{};
	...
	for (auto& [module_name, vclsid,viid, vtbl] : vtables) {
		for (auto& [func_name, iidfunc, vtblfunc, address] : find_function_by_clsid(clsid))
		{
			if (vtblfunc == vtbl && iidfunc == viid)
			{
				
				_logger.log_info_dml(std::format(L"- module: <link cmd=\"!cometa showm {0}\">{0}</link>,VTable offset:<link cmd=\"dps {0}+{1:#x}\">{1:#x}</link>.Name:<link cmd=\"x {0}!{2}\">{2}</link>,Address offset:{3:#x},<link cmd=\"bp {0}+{3:#x}\">BP</link>\n", module_name, vtbl, func_name, address));
				NewSyntheticSymbolInModule(module_name, address, sizeof(ULONG_PTR), func_name);
			}
		}
	}
