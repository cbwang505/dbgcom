        ...
        if (save_in_database) {
            if (ULONG64 base_addr{}; SUCCEEDED(_dbgsymbols->GetModuleByOffset2(
                ...
                if (auto vmi{ get_module_info(base_addr) }; std::holds_alternative<module_info>(vmi)) {
                    ...
                    ULONG64 vtbl_offset = vtable_addr - base_addr;
					std::wstring indexstr=std::format(L"module:{0},VTable offset:{1:#x}, <link cmd=\"!cometa index {0}\">Index Typelib?</link>\n", mi.name, vtbl_offset);
                        _logger.log_info_dml(indexstr);
                    _cometa.save_module_vtable({ mi.name, mi.timestamp, _cc.is_64bit() }, { clsid, iid, vtbl_offset });
                } else {
...
void comonitor::handle_module_load(std::wstring_view module_name, ULONG module_timestamp, ULONG64 module_base_addr) {
    ...
    std::wstring  modpath=_cometa.resolve_module_path_by_name(module_name.data());
    if(modpath.find(L"C:\\Windows")!= std::string::npos)
    {
        return;
    }
...
void comonitor::log_com_call_success(const CLSID& clsid, const IID& iid, std::wstring_view caller_name, ULONG64 objaddr) {
    ULONG tid{};
    ...
    if (objaddr)
    {
		_logger.log_info_dml(std::format(L"<col fg=\"normfg\">{}:{:03} [{}] CLSID: <b>{:b} ({})</b>, IID: <b>{:b} "
			L"({})</b></col> VTable: <link cmd=\"dps {:#x}\">{:#x}</link> <col fg=\"srccmnt\">SUCCESS (0x0)</col>",
			_process_id, tid, caller_name, clsid, clsid_name ? *clsid_name : L"N/A", iid,
			iid_name ? *iid_name : L"N/A", objaddr, objaddr));
    }
    else {
