...
namespace comon_ext
{
	...
	class cometa
	{
		...
		void add_symbol_by_clsid_and_iid(const CLSID& clsid, const IID& iid);
		void fill_known_iids();
		void fill_synthetic_symbols();
		...
		void insert_cotype_function(const cofunction& func);
	...
	public:
		...
		std::wstring resolve_module_path_by_name(std::wstring nameorpath);
		explicit cometa(IDebugControl4* dbgcontrol, bool is_wow64, const fs::path& db_path, bool create_new);
		BOOLEAN NewSyntheticSymbol(ULONG64 offset, ULONG64 sym_size, const std::wstring& sym_name);
		BOOLEAN NewSyntheticSymbolInModule(const std::wstring& mod_name, ULONG64 offset, ULONG64 sym_size, const std::wstring& sym_name);
		...
		std::vector<std::tuple<std::wstring, IID, ULONG64, ULONG64>> find_function_by_clsid(const CLSID& clsid);
		std::vector<std::tuple<std::wstring, CLSID, IID, ULONG64>> find_vtables_by_clsid_and_iid(const CLSID& clsid, const IID& iid);
		std::vector<std::tuple<std::wstring, CLSID, IID, ULONG64>> find_vtables_all();
