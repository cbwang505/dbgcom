...
void cometa_showc(wil::com_ptr_t<IDebugControl4> dbgcontrol, comon_ext::cometa& cometa, const CLSID& clsid) {
    ...
    for (auto& [module_name, iid, vtbl] : cometa.find_vtables_by_clsid(clsid)) {
        ...
        dbgcontrol->ControlledOutputWide(DEBUG_OUTCTL_AMBIENT_DML, DEBUG_OUTPUT_NORMAL, std::format(
            L"- module: <link cmd=\"!cometa showm {0}\">{0}</link>, IID: <link cmd=\"!cometa showi {1:b}\">{1:b}</link> ({2}), VTable offset: <link cmd=\"dps {0}+{3:#x}\">{3:#x}</link>\n",
            module_name, iid, iid_name ? *iid_name : L"N/A", vtbl).c_str());
        for (auto& [func_name, iidfunc, vtblfunc, address] : cometa.find_function_by_clsid(clsid))
        {
            if (vtblfunc == vtbl && iidfunc== iid)
            {
                dbgcontrol->ControlledOutputWide(DEBUG_OUTCTL_AMBIENT_DML, DEBUG_OUTPUT_NORMAL,
                    std::format(L"- module: <link cmd=\"!cometa showm {0}\">{0}</link>,VTable offset:<link cmd=\"dps {0}+{1:#x}\">{1:#x}</link>.Name:<link cmd=\"x {0}!{2}\">{2}</link>,Address offset:{3:#x},<link cmd=\"bp {0}+{3:#x}\">BP</link>\n", module_name, vtbl, func_name, address).c_str());
                cometa.NewSyntheticSymbolInModule(module_name, address, sizeof(ULONG_PTR), func_name);
            }
        }
...
void cometa_showm(wil::com_ptr_t<IDebugControl4> dbgcontrol, comon_ext::cometa& cometa, const std::wstring& module_name) {
    dbgcontrol->ExecuteWide(DEBUG_OUTCTL_THIS_CLIENT |
        DEBUG_OUTCTL_OVERRIDE_MASK |
        DEBUG_OUTCTL_NOT_LOGGED, std::format(L"lm vm {0}", module_name).c_str(), DEBUG_EXECUTE_ECHO);
